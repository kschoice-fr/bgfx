...
#define BGFX_CHECK_ENCODER0()                               \
	...
	void requestSuspend()
	{
		if (s_ctx->m_singleThreaded) {
			...
			g_suspendRequested = false;
			g_suspendComplete = true;
		} else {
			g_suspendRequested = true;
			g_suspendComplete = false;
		}
		g_resumeRequested = false;
		g_resumeComplete = false;
	}
